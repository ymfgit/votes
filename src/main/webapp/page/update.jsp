...
<div id="voteManage" class="box">
	<h2>添加新投票</h2>
	<div class="content">
	<form method="post" action="subject_update.action">
			<dl>
				<dt>投票内容：</dt>
				<dd>
				   <input type="hidden" name="vsId" value="${sessionScope.voteBean.vsId }"/>
				   <input type="text" class="input-text" name="vsTitle"  value="${sessionScope.voteBean.vsTitle }"/>
				</dd>
				<dt>投票类型：</dt>
				<dd>
		  		   <input type="radio" name="vsType"  checked="checked" value="${sessionScope.voteBean.vsType }" />单选
				   <input type="radio" name="vsType" value="${sessionScope.voteBean.vsType }" />多选
				</dd>
				<dt>投票选项：</dt>
				
				<dd id="voteoptions">
					<c:forEach items="${sessionScope.voteBean.options }" var="option">
					  <p><input type="text" class="input-text" name="options" value="${option.getVooption()}"/></p>
				   </c:forEach>
				</dd>
